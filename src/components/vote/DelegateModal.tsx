...
export default function DelegateModal({ isOpen, onDismiss, title }: VoteModalProps) {
  ...
  const PHIBalance = useTokenBalance(account ?? undefined, chainId ? PHI[chainId] : undefined)
      ...
      {!attempting && !hash && (
        <ContentWrapper gap="lg">
          <AutoColumn gap="lg" justify="center">
            ...
            <TYPE.body>Earned PHI tokens represent voting shares in Phidex governance.</TYPE.body>
            <TYPE.body>
      ...
      {attempting && !hash && (
        <LoadingView onDismiss={wrappedOndismiss}>
          <AutoColumn gap="12px" justify={'center'}>
            ...
            <TYPE.main fontSize={36}>{PHIBalance?.toSignificant(4)}</TYPE.main>
          </AutoColumn>
        </LoadingView>
      )}
      {hash && (
