...
import { PHI, ZERO_ADDRESS } from '../../constants'
import { JSBI, TokenAmount, ChainId } from '@Phidex/sdk'
...
export default function Vote() {
  ...
  const PHIBalance: TokenAmount | undefined = useTokenBalance(account ?? undefined, chainId ? PHI[chainId] : undefined)
  const userDelegatee: string | undefined = useUserDelegatee()

  // show delegation option if they have have a balance, but have not delegated
  const showUnlockVoting = Boolean(
    PHIBalance && JSBI.notEqual(PHIBalance.raw, JSBI.BigInt(0)) && userDelegatee === ZERO_ADDRESS
  )
  ...
  return (
    <PageWrapper gap="lg" justify="center">
      ...
      <TopSection gap="md">
        <VoteCard>
          ...
          <CardSection>
            <AutoColumn gap="md">
              <RowBetween>
                <TYPE.white fontWeight={600}>Phidex Governance</TYPE.white>
              </RowBetween>
              <RowBetween>
                <TYPE.white fontSize={14}>
                  PHI tokens represent voting shares in Phidex governance. You can vote on each proposal yourself or
                  delegate your votes to a third party.
                </TYPE.white>
              </RowBetween>
              <ExternalLink
                style={{ color: 'white', textDecoration: 'underline' }}
                href="https://Phidex.io/blog/PHI"
                target="_blank"
              >
                <TYPE.white fontSize={14}>Read more about Phidex governance</TYPE.white>
              </ExternalLink>
            </AutoColumn>
          </CardSection>
          <CardBGImage />
      ...
      <TopSection gap="2px">
        <WrapSmall>
          ...
          {showUnlockVoting ? (
            ...
            JSBI.notEqual(JSBI.BigInt(0), PHIBalance?.raw) ? (
            <TYPE.body fontWeight={500} mr="6px">
              <FormattedCurrencyAmount currencyAmount={PHIBalance} /> Votes
            </TYPE.body>
          ) : (
            ''
          )}
      ...
      <TYPE.subHeader color="text3">
        A minimum threshhold of 1% of the total PHI supply is required to submit proposals
      </TYPE.subHeader>
    </PageWrapper>
  )
}
