...
import { TokenAmount, JSBI, ChainId } from '@Phidex/sdk'
...
function fetchClaim(account: string, chainId: ChainId): Promise<UserClaimData | null> {
  ...
  return (CLAIM_PROMISES[key] =
    CLAIM_PROMISES[key] ??
    fetch('https://merkle-drop-1.Phidex.workers.dev/', {
...
// check if user is in blob and has not yet claimed PHI
...
export function useUserUnclaimedAmount(account: string | null | undefined): TokenAmount | undefined {
  ...
  if (!canClaim || !userClaimData) {
    return new TokenAmount(PHI, JSBI.BigInt(0))
  }
  return new TokenAmount(PHI, JSBI.BigInt(userClaimData.amount))
}
      ...
      return distributorContract
        ...
        .then((response: TransactionResponse) => {
          addTransaction(response, {
            summary: `Claimed ${unClaimedAmount?.toSignificant(4)} PHI`,
