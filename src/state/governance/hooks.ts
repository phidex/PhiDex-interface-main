import { PHI, PRELOADED_PROPOSALS } from './../../constants/index'
import { TokenAmount } from '@Phidex/sdk'
import { isAddress } from 'ethers/lib/utils'
import { useGovernanceContract, usePHIContract } from '../../hooks/useContract'
...
import { abi as GOV_ABI } from '@Phidex/governance/build/GovernorAlpha.json'
    ...
    return allProposals
      ...
      .map((p, i) => {
        ...
        const formattedProposal: ProposalData = {
          ...
          forCount: parseFloat(ethers.utils.formatPHIts(allProposals[i]?.result?.forVotes.toString(), 18)),
          againstCount: parseFloat(ethers.utils.formatPHIts(allProposals[i]?.result?.againstVotes.toString(), 18)),
...
export function useUserDelegatee(): string {
  ...
  const PHIContract = usePHIContract()
  const { result } = useSingleCallResult(PHIContract, 'delegates', [account ?? undefined])
...
export function useUserVotes(): TokenAmount | undefined {
  ...
  const votes = useSingleCallResult(PHIContract, 'getCurrentVotes', [account ?? undefined])?.result?.[0]
  return votes && PHI ? new TokenAmount(PHI, votes) : undefined
}
...
export function useUserVotesAsOfBlock(block: number | undefined): TokenAmount | undefined {
  ...
  // check for available votes
  const PHI = chainId ? PHI[chainId] : undefined
  const votes = useSingleCallResult(PHIContract, 'getPriorVotes', [account ?? undefined, block ?? undefined])
    ?.result?.[0]
  return votes && PHI ? new TokenAmount(PHI, votes) : undefined
}
...
export function useDelegateCallback(): (delegatee: string | undefined) => undefined | Promise<string> {
  ...
  return useCallback(
    (delegatee: string | undefined) => {
      ...
      if (!PHIContract) throw new Error('No PHI Contract!')
      return PHIContract.estimateGas.delegate(...args, {}).then(estimatedGasLimit => {
        return PHIContract
    ...
    [account, addTransaction, chainId, library, PHIContract]
  )
}
